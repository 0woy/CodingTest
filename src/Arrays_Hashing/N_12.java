...
public class N_12{
    ...
    public static void initMap(){
        map = new HashMap<>();
        StringTokenizer symbol = new StringTokenizer("I IV V IX X XL L XC C CD D CM M", " ");
        StringTokenizer value = new StringTokenizer("1 4 5 9 10 40 50 90 100 400 500 900 1000", " ");
        while(symbol.hasMoreTokens()){
            map.put(Integer.parseInt(value.nextToken()), symbol.nextToken());
        }
    }
    public static String intToRoman(int num) {
        initMap();
        int len = String.valueOf(num).length()-1; //3749 = 3자리 -> 700
        StringBuilder sb = new StringBuilder();
        while(num>0){
            int size = (int)Math.pow(10,len);
            int cur =num/size; //3000
            if(map.containsKey(cur*size)){
                sb.append(map.get(cur*size));
            }
            else{
                if(cur>5){
                    sb.append(map.get(size*5));
                    cur-=5;
                }
                while(cur>0){
